  ...
  return (
    <div className="space-y-2">
        ...
        {items.map((it, i) => (
          <li key={i} className="py-2 flex gap-2 items-center">
            ...
            <input
              ...
              placeholder="cal"
              className="w-20 px-2 py-2 rounded bg-neutral-900 border border-neutral-800 text-sm text-right tabular-nums"
            />
            <input
              type="number" inputMode="decimal" step="1"
              value={it.protein_g ?? ""}
              onChange={e => onUpdate(i, {
                protein_g: e.target.value === "" ? null : parseFloat(e.target.value),
              })}
              placeholder="p"
              className="w-14 px-2 py-2 rounded bg-neutral-900 border border-neutral-800 text-sm text-right tabular-nums"
            />
            <button
              onClick={() => onRemove(i)}
      ...
      <div className="text-xs text-neutral-400 tabular-nums">
        total: {Math.round(totalCal)} cal · {Math.round(totalProtein)}g protein
      </div>
