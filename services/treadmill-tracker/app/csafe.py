...
Standard CSAFE frame: 0xF1 <payload> <checksum> 0xF2.
Inside payload, control bytes 0xF1/0xF2/0xF3 are escaped as
0xF3 followed by (byte XOR 0x80). Checksum is XOR of unescaped
payload bytes.
"""
...
def decode(frame: bytes) -> bytes:
    if len(frame) < _MIN_FRAME_LEN or frame[0] != START or frame[-1] != END:
